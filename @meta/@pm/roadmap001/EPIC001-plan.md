...
- [ ] **Phase 1.1 — Freeze the additive public contract**
...
- [ ] **Phase 1.2 — Retain provider pagination capability in Rust**
...
- [ ] **Phase 1.3 — Implement one native/fallback page operation**
...
- [ ] **Phase 1.4 — Expose the safe Elixir API**
...
- [ ] **Phase 1.5 — Prove provider-neutral behavior without credentials**
...
- [ ] **Phase 1.6 — Add optional S3-compatible verification and documentation**
...
- [ ] **Phase 1.7 — Complete cumulative quality and package verification**
...
- [ ] The new operation is additive and all legacy list contracts remain unchanged.
- [ ] Elixir option validation covers unknown keys, prefix, `max_keys`, and `page_token` without NIF panics.
- [ ] Native cloud providers perform one `PaginatedListStore::list_paginated` request per public page call.
- [ ] Wasabi is supported through the ordinary S3-compatible endpoint path without provider-specific protocol code.
- [ ] Memory/local compatibility pages are deterministic and explicitly documented as full-level materialization.
- [ ] `max_keys` bounds objects and prefixes together for empty, partial, full, and exact-boundary pages.
- [ ] Following tokens over an unchanged listing returns every immediate entry exactly once and terminates with `nil`.
- [ ] Invalid fallback tokens and provider errors return through documented ObjectStoreX error shapes.
- [ ] Existing `list_with_delimiter/2` and `ObjectStoreX.Stream.list_stream/2` tests pass unchanged.
- [ ] Credential-free tests never contact S3, Wasabi, Azure, GCS, or another external service.
- [ ] Optional cloud coverage uses a unique prefix, cleans up in failure paths, and does not print secrets/tokens.
- [ ] `OBJECTSTOREX_BUILD=1 mix compile --warnings-as-errors` passes.
- [ ] `mix format --check-formatted`, credential-free `mix test`, and `mix docs` pass.
- [ ] `cargo fmt --check`, locked all-feature tests, strict Clippy, and locked release build pass.
- [ ] `mix hex.build`, `bin/qa_check.sh`, and `git diff --check` pass.
- [ ] Package contents include the changed native and Elixir sources but no generated docs, credentials, or transient artifacts.
- [ ] Evidence is complete and the epic commit title is exactly `[Listing] Add paginated delimiter pages`.
